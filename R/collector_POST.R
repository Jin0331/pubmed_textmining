...
if(length(arg) == 0){
    ...
    term <- read_delim(file = "pubmed_textmining/dict/term_dict.txt") %>% 
        filter(Cancer_Type == arg[1]) %>% 
    ...
    # disease filter
    ## mesh id mapping
    disease_dict <- read_delim(file = "pubmed_textmining/dict/ctd_dict_medic.txt") %>% 
