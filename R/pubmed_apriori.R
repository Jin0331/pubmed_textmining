...
if(length(arg) == 0){
  ...
  # pubmed collect
  if(file.exists(paste0("RAW_DATA/", arg[1], "_gene_disease_pair.txt")) == FALSE){
    ...
    title_abstract %>% dplyr::select(pmid, year, journal, authors,title, abstract) %>% 
      write_delim(file = paste0("RAW_DATA/", arg[1], "_pubmed.txt"), delim = "\t")
    
    # gene-disease import
    gene_disease_filter %>% 
      write_delim(file = paste0("RAW_DATA/", arg[1], "_gene_disease_pair.txt"), delim = "\t")
  ...
  cancer_type <- arg[1]
  item_table <- read_delim(file = paste0("RAW_DATA/", cancer_type,"_gene_disease_pair.txt"), delim = "\t",
